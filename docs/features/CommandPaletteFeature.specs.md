...
- opening built-in pages (e.g. `chiaroscuro://settings`).

This is the primary navigation interface — there is no persistent address bar.

## Terminology

- **Command palette**: the command input UI (Arc-style, invoked via Ctrl-K).
...
- **Built-in page**: an app-defined page like `chiaroscuro://settings`.
...
- If the command is a built-in page address (like `chiaroscuro://settings`), it must open the corresponding app page.
...
- The command is interpreted in this order:
  ...
  2. If the command is a built-in page (e.g. `chiaroscuro://settings`), open that page.
...
- **Ctrl-K conflict**: Some web apps use Ctrl-K for their own command palettes or link insertion. Need to decide if the browser shortcut should always take precedence or if there should be a way to pass it through to the page.
